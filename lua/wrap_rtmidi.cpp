...
extern "C" {
	int LUA_API luaopen_rtmidi (lua_State *L);
}
...
{
	RtMidiIn* midi_in = checkMidiIn(L, 1);
	std::vector<unsigned char> message(3);

	double delta = midi_in->getMessage(&message);
	if(message.size() == 3)
	{
...
static void set_info (lua_State *L)
{
	...
	lua_pushliteral (L, "Copyright (C) 2007-2014 LuaMidi");
	lua_settable (L, -3);
	lua_pushliteral (L, "_DESCRIPTION");
	lua_pushliteral (L, "Provides a simple interface for MIDI I/O");
	lua_settable (L, -3);
	lua_pushliteral (L, "_VERSION");
	lua_pushliteral (L, "LuaMIDI 0.12");
	lua_settable (L, -3);
}
...
static int MidiOut_register(lua_State* L)
{
	...
	lua_newtable(L);
	luax_setfuncs(L, MidiOut_meta, 0);
	lua_rawset(L, -3);
	lua_pop(L, 1);
	return 1;
}
...
int LUA_API luaopen_rtmidi (lua_State *L) {
	...
	// returning userdata gives us a nice scoped gc
	lua_newtable(L);
	luax_setfuncs(L, luamidi_methods, 0);
	set_info(L);
